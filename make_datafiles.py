...
def write_to_mindrecord(url_file, out_file, makevocab=False):
    ...
    writer = FileWriter(file_name=out_file, shard_num=(num_stories // CHUNK_SIZE) + 1)
    schema_json = {"article": {"type": "bytes"}, "abstract": {"type": "bytes"}}
    ...
    for idx, s in enumerate(story_fnames):
        ...
        # Write to mindrecord
        writer.write_raw_data([{'article': article.encode(), 'abstract': abstract.encode()}])
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("USAGE: python make_datafiles.py <cnn_stories_dir> <dailymail_stories_dir> <save_dir>")
        sys.exit()
    ...
    cnn_tokenized_stories_dir = os.path.join(
        save_dir, cnn_tokenized_stories_dir)
    dm_tokenized_stories_dir = os.path.join(save_dir, dm_tokenized_stories_dir)
    finished_files_dir = os.path.join(save_dir, finished_files_dir)
    ...
    write_to_mindrecord(all_test_urls, os.path.join(
        finished_files_dir, "test.mindrecord"))
    write_to_mindrecord(all_val_urls, os.path.join(
        finished_files_dir, "val.mindrecord"))
    write_to_mindrecord(all_train_urls, os.path.join(
        finished_files_dir, "train.mindrecord"), makevocab=True)
